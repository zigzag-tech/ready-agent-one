...
import {
  ...
  addToPlayerFocusedRange,
  playerTargets,
  removeFromPlayerAttackRange,
  removeFromPlayerCloseRange,
  removeFromPlayerFocusedRange,
...
export const usePlayerPhysics = ({
  ...
  const onLargeCollideEnd = useCallback(
    (
      { mobID, mobVariant }: { mobID: number; mobVariant: MOB_VARIANT },
      fixtureIndex: number
    ) => {
      if (fixtureIndex === 0) {
        if (mobVariant === MOB_VARIANT.large) {
      ...
      fixtures: [
        {
          ...
          fixtureOptions: {
            density: 20,
            filterCategoryBits:
              COLLISION_FILTER_GROUPS.player | COLLISION_FILTER_GROUPS.physical,
          },
        },
      ],
    }),
