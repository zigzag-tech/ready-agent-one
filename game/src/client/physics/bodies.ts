import { BodyDef, Body, Box, Circle, FixtureOpt, Vec2 } from "planck";
...
export const addBody = ({
  ...
  if (!body) {
    ...
    if (fixtures && fixtures.length > 0) {
      fixtures.forEach(
        ({ shape, fixtureOptions, hx, hy, radius, center }, fixtureIndex) => {
          fixtureOptions = {
            ...fixtureOptions,
            userData: {
              uuid,
    ...
    if (fixture) {
      ...
      if (
        ...
        fixture.setFilterData({
          ...
          categoryBits:
            categoryBits !== undefined ? categoryBits : originalCategoryBits,
