  ...
  useBody(
    () => ({
      ...
      fixtures: walls.map((wall) => {
        const horizontal =
          wall.direction === RoomDirection.NORTH ||
          wall.direction === RoomDirection.SOUTH;
        const xLength = horizontal
          ? Math.abs(wall.start[0] - wall.end[0])
          : 1.2;
        const yLength = horizontal
          ? 1.2
          : Math.abs(wall.start[1] - wall.end[1]);
